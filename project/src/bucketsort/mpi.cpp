//
// Created by Yohandi on 2023/11/17.
...
void bucketSort(std::vector<int> &vec, int num_buckets, int numtasks,
  ...
  int range_bucket = range_to[num_buckets_to[taskid] - 1] -
                     range_from[num_buckets_from[taskid]];

  std::vector<int> range_bucket_from(current_num_buckets, 0),
      range_bucket_to(current_num_buckets, 0);
  ...
  for (int num : vec) {
    if (num < min_val + range_from[num_buckets_from[taskid]] ||
        num >= min_val + range_to[num_buckets_to[taskid] - 1])
      continue;
    ...
    if (num - min_val < range_bucket_leftover * (range_bucket_chunk + 1)) {
      ...
      index =
          range_bucket_leftover +
          (num - range_bucket_from[range_bucket_leftover]) / range_bucket_chunk;
    }
  ...
  if (taskid == MASTER) {
    ...
    for (int i = 1; i < numtasks; ++i) {
      ...
      MPI_Recv(&vec[local_size], local_n, MPI_INT, i, 0, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);
