...
void parallelMerge(std::vector<int> &vec, int start, std::vector<int> &vec_1,
                   int l_1, int r_1, std::vector<int> &vec_2, int l_2,
                   int r_2) {
...
#pragma omp section
      parallelMerge(vec, start + m1 + m2, vec_1, l_1 + m1, r_1, vec_2, l_2 + m2,
                    r_2);
    }
  }
}

void copyChunk(const std::vector<int> &src, std::vector<int> &dest, int start,
               int end, int offset) {
...
void parallelCopy(const std::vector<int> &src, std::vector<int> &dest, int l,
                  int r) {
  int n = r - l + 1;
  ...
  for (int i = 0; i < pool.size(); ++i) {
    ...
    futures.emplace_back(
        pool.enqueue(copyChunk, std::ref(src), std::ref(dest), start, end, l));
  }
...
void mergeSortParallel(std::vector<int> &vec, int l, int r, int depth) {
  if (l < r) {
    ...
    if (depth > 0) {
      auto fut_1 =
          pool.enqueue(mergeSortParallel, std::ref(vec), l, m, depth - 1);
