...
void oddEvenSort(std::vector<int> &vec, int numtasks, int taskid,
                 MPI_Status *status) {
  int n = (int)vec.size();
  ...
  while (true) {
    ...
    if (end != n && end % 2 == 0) {
      ...
      MPI_Recv(&vec[end], 1, MPI_INT, taskid + 1, 0, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);
    ...
    if (start != 0 && start % 2 == 0) {
      MPI_Recv(&vec[start - 1], 1, MPI_INT, taskid - 1, 0, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);
  ...
  if (taskid == MASTER) {
    for (int i = 1; i < numtasks; ++i) {
      MPI_Recv(&vec[from[i]], to[i] - from[i], MPI_INT, i, 0, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);
    }
  } else {
